...
function VisioForge_UninstallRedist {
    $toolsPath = split-path -parent $PSScriptRoot
    $ErrorLog = $toolsPath + "\UninstallLog.txt"
    
    Try {
        Remove-ItemProperty -Path "HKLM:\Software\VisioForge\" -Name "NuGetRedist"
        Remove-Item -Path "HKLM:\SOFTWARE\" -Name VisioForge   
    }
    Catch {
    } 
    ...
    Try { 
        ...
        Write-Output $CMD
        Start-Process -FilePath $CMD -ArgumentList "/exenoui /qn /x //" -Wait
...
Export-ModuleMember -Function VisioForge_UninstallRedist
